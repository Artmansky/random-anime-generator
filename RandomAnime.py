...
def main():
  ...
  mainTitle = TK.Label(win,text="Input your MAL/AL nickname below!",font=40,width=40)
  mainTitle.pack(pady=25)
  userEntry = TK.Entry(win,width=40)
  userEntry.pack(pady=10)
  buttonMAL = TK.Button(win,text="Load MAL List",width=15,command=lambda:{queryReqMAL(userEntry.get()),userEntry.delete(0,"end")})
  buttonMAL.place(x=180,y=120)
  buttonAL = TK.Button(win,text="Load AL List",width=15,command=lambda:{queryReqAL(userEntry.get()),userEntry.delete(0,"end")})
  buttonAL.place(x=305,y=120)
  buttonListMAL = TK.Button(win,text="Load Last MAL List",width=15,command=lambda:grabFromFile("listMAL.xml"))
  buttonListMAL.place(x=180,y=150)
  buttonListAL = TK.Button(win,text="Load Last AL List",width=15,command=lambda:grabFromFile("listAL.xml"))
  buttonListAL.place(x=305,y=150)
  warning = TK.Label(win,text="DISCLAMER: Scrapping MAL list for the first time takes long time due to site's old API!",fg="red")
