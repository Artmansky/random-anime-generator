...
from tkinter import messagebox as TKmess
...
def grabFromFile(filename):
  ...
  try:
    ...
    TK.messagebox.showerror("Error!", "Last anime list doesn't exist!")
    return
  ...
  msgBox = TKmess.askquestion(
    "Result",
    "Your next anime is: "
    + titleArray[rand]
    + ". Would you like to open anime's site?",
  )
  if (msgBox) == "yes":
    WEB.open(urls[rand])
...
def queryReqAL(name):
  ...
  """
    ...
    try:
      for anime in data_json["data"]["MediaListCollection"]["lists"][0]["entries"]:
        if anime["media"]["status"]!="NOT_YET_RELEASED":
      ...
      TK.messagebox.showerror("Error!", "No user found! Check spelling!")
      return
  ...
  if not name:
    TKmess.showerror("Name not given!", "Input field is empty!")
...
def queryReqMAL(name):
  if name:
    ...
    url = "https://myanimelist.net/animelist/{}/load.json?status=7&offset=0".format(
      name
    )
    ...
    offset = 300
    while True:
      url = "https://myanimelist.net/animelist/{}/load.json?status=7&offset={}".format(
        name, offset
      )
      response = REQ.get(url)
    ...
    for anime in data_json:
      if (int(anime["status"]) == 6) and (int(anime["anime_airing_status"])!=3):
        newAnime = ET.SubElement(root, "anime")
        ET.SubElement(newAnime, "series_title").text = str(anime["anime_title"])
        ET.SubElement(newAnime, "my_url").text = ("https://myanimelist.net" + str(anime["anime_url"]))
    # Element Tree is saved to the file and method to pick anime is executed
    save = ET.ElementTree(root)
    save.write("listMAL.xml")
    grabFromFile("listMAL.xml")
  if not name:
    TK.messagebox.showerror("Name not given!", "Input field is empty!")
...
def main():
  ...
  mainTitle = TK.Label(
    win, text="Input your MAL/AL nickname below!", font=40, width=40
  )
  mainTitle.pack(pady=25)
  userEntry = TK.Entry(win, width=40)
  userEntry.pack(pady=10)
  buttonMAL = TK.Button(
    win,
    text="Load MAL List",
    width=15,
    command=lambda: {queryReqMAL(userEntry.get()), userEntry.delete(0, "end")},
  )
  ...
  buttonAL = TK.Button(
    ...
    text="Load AL List",
    width=15,
    command=lambda: {queryReqAL(userEntry.get()), userEntry.delete(0, "end")},
  )
  buttonAL.place(x=305, y=120)
  buttonListMAL = TK.Button(
    win,
    text="Load Last MAL List",
    width=15,
    command=lambda: grabFromFile("listMAL.xml"),
  )
  buttonListMAL.place(x=180, y=150)
  buttonListAL = TK.Button(
    win,
    text="Load Last AL List",
    width=15,
    command=lambda: grabFromFile("listAL.xml"),
  )
  buttonListAL.place(x=305, y=150)
  warning = TK.Label(
    win,
    text="DISCLAMER: Scrapping MAL list for the first time takes long time due to site's old API!",
    fg="red",
  )
